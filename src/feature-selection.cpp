...
Eigen::Matrix< double, 2, Eigen::Dynamic > FeatureSelection::Ssc(
...
void FeatureSelection::detectFeatures(const uint32_t numberCandidate)
{
    ...
    for(int i(0); i<m_imgGray->rows; i++)
    {
        for(int j(0); j<m_imgGray->cols; j++)
        {
            ...
            {
                keyPoints.emplace_back(cv::KeyPoint(cv::Point2f(j, i), 1.0, m_gradientOrientation.at<float>(i, j), m_gradientMagnitude.at<float>(i, j)));
            }
        }
    }
    ...
    for(int i(0); i<imgHSV.rows; i++)
    {
        for(int j(0); j<imgHSV.cols; j++)
        {
            if (m_gradientMagnitude.at<float>(i, j) >= minMagnitude)
            {
                ...
                cv::Scalar color = generateColor(minMagnitude, 255.0, m_gradientMagnitude.at<float>(i, j) - minMagnitude );
                px[0] = color[0];
                px[1] = color[1];
                px[2] = color[2];
            }
